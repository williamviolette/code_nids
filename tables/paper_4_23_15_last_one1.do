...
program define reduced_form
	* REDUCED FORM *
	...
	outreg2 using clean/tables/health, nonotes tex(frag) sortvar(h_ch h_ch_large h_chS h_chS_large large lo lo_large) keep(h_ch h_ch_large h_chS h_chS_large large lo lo_large) label replace  nocons  addtext(Large Controls, NO, Treated Area, Over 5)  addstat(Mean, r(mean)) 	addnote("All regressions control for lagged quartiles in outcomes")	
	xi: reg zhfa_ch h_ch h_ch_large  h_chS_large h_chML_large  m_ch lo lo_large lo2 lo2_large m_ch_large a sex h_chS h_chML i.rl1*i.prov  zhfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.5 & size_lag>2, robust cluster(cluster)	
	quietly sum zhfa, detail
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large  h_chS h_chS_large lo lo_large ) label append nocons  addtext(Large Controls, YES, Treated Area, Over 5)  addstat(Mean, r(mean)) 	addnote("All regressions control for lagged quartiles in outcomes")	

	xi: reg zwfa_ch h_ch h_ch_large  h_chS_large h_chML_large  m_ch lo lo_large lo2 lo2_large m_ch_large large  a sex  h_chS h_chML  i.r*i.prov  zwfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.66 & size_lag>2, robust cluster(cluster)	
	quietly sum zwfa, detail
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large h_chS h_chS_large lo lo_large) label append nocons  addtext(Large Controls, NO, Treated Area, Over 5)  addstat(Mean, r(mean)) 	addnote("All regressions control for lagged quartiles in outcomes")	
	xi: reg zwfa_ch h_ch h_ch_large  h_chS_large h_chML_large  m_ch lo lo_large lo2 lo2_large m_ch_large   a sex  h_chS h_chML  i.rl1*i.prov  zwfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.66 & size_lag>2, robust cluster(cluster)	
	quietly sum zwfa, detail
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large h_chS h_chS_large lo lo_large ) label append nocons  addtext(Large Controls, YES, Treated Area, Over 5)  addstat(Mean, r(mean)) 	addnote("All regressions control for lagged quartiles in outcomes")	

	xi: reg c_health_ch h_ch h_ch_large  h_chS_large h_chML_large  m_ch lo lo_large lo2 lo2_large m_ch_large large   h_chS h_chML    i.r*i.prov  a sex c_health_p* if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(cluster)	
...
*	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large) label append nocons  addtext(Large Controls, YES, Treated Area, Over 5)  addstat(Mean, r(mean)) 	addnote("All regressions control for lagged quartiles in outcomes")	
	...
	xi: reg c_ill_ch h_ch h_ch_large  h_chS_large h_chML_large  m_ch lo lo_large lo2 lo2_large m_ch_large large a sex c_ill_p*   h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(cluster)	
