...
program define house_quality
	...
	xi: reg wath_ch h_ch h_ch_large *_LARGE large m_ch lo lo_large m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg toih_ch h_ch h_ch_large *_LARGE large m_ch lo lo_large m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg walls_b_ch h_ch h_ch_large *_LARGE large m_ch lo lo_large m_ch_large  h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)
	xi: reg refuse_ch h_ch h_ch_large *_LARGE large m_ch lo lo_large m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)
	xi: reg mktv_ch h_ch h_ch_large *_LARGE large m_ch lo lo_large m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)
	xi: reg rooms_ch h_ch h_ch_large *_LARGE large m_ch lo lo_large m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)
...
*	xi: reg h_freqmdr_ch h_ch  m_ch lo lo_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
*	outreg2 using clean/tables/n_quality, nonotes tex(frag) keep(h_ch ) label append nocons  addtext(Treated Area, Over 5)
...
***** INCLUDES INTERACTIONS
	xi: reg h_freqdomvio_ch h_ch h_ch_large *_LARGE large m_ch lo lo_large m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg h_freqvio_ch h_ch h_ch_large *_LARGE large m_ch lo lo_large m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg h_freqdrug_ch h_ch h_ch_large *_LARGE large m_ch lo lo_large m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/n_quality_i, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)
	xi: reg n_trust_ch h_ch h_ch_large *_LARGE large m_ch lo lo_large m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/n_quality_i, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)
	xi: reg n_stay_ch h_ch h_ch_large *_LARGE large m_ch lo lo_large m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/n_quality_i, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)
	...
	** TEST THAT PERCEPTIONS ARE THE SAME AT BASELINE
	...
	outreg2 using clean/tables/n_quality_t, nonotes tex(frag) keep(large1 ) label append nocons  addtext(Treated Area, Over 5)
		...
		** LEVELS
*	xi: reg wath h_ch h_ch_crowd crowd_lag m_ch lo lo_large m_ch_crowd  h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
...
*	xi: reg toih h_ch h_ch_crowd crowd_lag m_ch lo lo_large m_ch_crowd  h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
...
*	xi: reg walls_b h_ch h_ch_crowd crowd_lag m_ch lo lo_large m_ch_crowd  h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
*	outreg2 using clean/tables/house_quality_levels, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
*	xi: reg rooms h_ch h_ch_crowd crowd_lag  m_ch lo lo_large m_ch_crowd h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
*	outreg2 using clean/tables/house_quality_levels, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
end
...
*	xi: reg c_absent_ch h_ch m_ch lo lo_large a sex h_chS h_chML    i.r*i.prov c_absent_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & size_lag>2, robust cluster(cluster)	
*	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	
*	xi: reg c_failed_ch h_ch m_ch lo lo_large a sex h_chS h_chML    i.r*i.prov c_failed_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & size_lag>2, robust cluster(cluster)	
*	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	
...
*	xi: reg zbmi_ch  h_ch m_ch lo lo_large h_chS h_chML    i.r*i.prov zbmi_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zbmi_ch<3.4 & zbmi_ch>-2.4 & size_lag>2, robust cluster(cluster)	
*	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	
...
program define first_stage
	...
	* FIRST STAGE *
	...
	xi: reg size_ch H_* sl_* mm_* h_chS h_chML lo lo_large lo2 lo2_large *_LARGE   i.r*i.prov  if im<=3500 & size_lag<$s  & hclust>=$clust, robust cluster(cluster)
	coefplot, vertical keep(H_*)
	graph export clean/tables/size_ch.pdf, as(pdf) replace

	xi: reg size_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large    i.r*i.prov  if  hclust>=0 & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	...
	xi: reg size_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	...
	xi: reg size_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large h_chS h_chML    i.r*i.prov  if  hclust>=10 & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	...
	xi: reg crowd_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg ad_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg ct_ch h_ch h_ch_large *_LARGE m_ch lo lo_large m_ch_large large h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
end
...
program define reduced_form
	* REDUCED FORM *
	...
	xi: reg zhfa_ch h_ch h_ch_large *_LARGE m_ch lo lo_large m_ch_large large a sex   h_chS h_chML i.r*i.prov  zhfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.5 & size_lag>2, robust cluster(cluster)	
	...
	xi: reg zhfa_ch h_ch h_ch_large *_LARGE m_ch lo lo_large m_ch_large a sex h_chS h_chML i.rl1*i.prov  zhfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.5 & size_lag>2, robust cluster(cluster)	
	quietly sum zhfa, detail
	...
	xi: reg zwfa_ch h_ch h_ch_large *_LARGE m_ch lo lo_large m_ch_large large  a sex  h_chS h_chML  i.r*i.prov  zwfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.66 & size_lag>2, robust cluster(cluster)	
	...
	xi: reg zwfa_ch h_ch h_ch_large *_LARGE m_ch lo lo_large m_ch_large   a sex  h_chS h_chML  i.rl1*i.prov  zwfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.66 & size_lag>2, robust cluster(cluster)	
	quietly sum zwfa, detail
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large) label append nocons  addtext(Large Controls, YES, Treated Area, Over 5)  addstat(Mean, r(mean)) 	addnote("All regressions control for lagged quartiles in outcomes")	

	xi: reg c_health_ch h_ch h_ch_large *_LARGE m_ch lo lo_large m_ch_large large   h_chS h_chML    i.r*i.prov  a sex c_health_p* if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(cluster)	
	quietly sum c_health, detail
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Large Controls, NO, Treated Area, Over 5)  addstat(Mean, r(mean)) 	addnote("All regressions control for lagged quartiles in outcomes")	
*	xi: reg c_health_ch h_ch h_ch_large *_LARGE m_ch lo lo_large m_ch_large    h_chS h_chML    i.rl1*i.prov  a sex c_health_p* if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(cluster)	
*	quietly sum c_health, detail
*	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large) label append nocons  addtext(Large Controls, YES, Treated Area, Over 5)  addstat(Mean, r(mean)) 	addnote("All regressions control for lagged quartiles in outcomes")	

	xi: reg c_ill_ch h_ch h_ch_large *_LARGE m_ch lo lo_large m_ch_large large a sex c_ill_p*   h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(cluster)	
	quietly sum c_ill, detail
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Large Controls, NO, Treated Area, Over 5)  addstat(Mean, r(mean)) 	addnote("All regressions control for lagged quartiles in outcomes")	
*	xi: reg c_ill_ch h_ch h_ch_large *_LARGE m_ch lo lo_large m_ch_large a sex c_ill_p*   h_chS h_chML    i.rl1*i.prov  if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(cluster)	
*	quietly sum c_ill, detail
*	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large) label append nocons  addtext(Large Controls, YES, Treated Area, Over 5)  addstat(Mean, r(mean)) 	addnote("All regressions control for lagged quartiles in outcomes")	
end		
...
program define income 
	...
	xi: reg pi_hhincome_ch h	_ch  m_ch lo lo_large   h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	...
	xi: reg pi_hhincome_p_ch h_ch   m_ch lo lo_large   h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	...
	outreg2 using clean/tables/inc, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg pi_hhwage_p_ch h_ch  m_ch lo lo_large   h_chS h_chML    i.r*i.prov  if   hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum pi_hhwage_p, detail
	outreg2 using clean/tables/inc, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg e_ch h_ch  m_ch lo lo_large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum e, detail
	outreg2 using clean/tables/inc, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg ue_ch h_ch   m_ch lo lo_large   h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum ue, detail
	outreg2 using clean/tables/inc, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)

	*** WITH INTERACTION
	xi: reg pi_hhincome_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum pi_hhincome, detail
	...
	xi: reg pi_hhincome_p_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum pi_hhincome_p, detail
	...
	xi: reg pi_hhwage_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum pi_hhwage, detail
	outreg2 using clean/tables/inc_i, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg pi_hhwage_p_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum pi_hhwage_p, detail
	outreg2 using clean/tables/inc_i, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg e_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum e, detail
	outreg2 using clean/tables/inc_i, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg ue_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum ue, detail
	outreg2 using clean/tables/inc_i, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
...
program define remit
	...
	xi: reg a_cr_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	...
	xi: reg a_cg_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	...
	xi: reg pi_hhremitt_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if pi_hhremitt_ch<12000 & pi_hhremitt_ch>-12000 &  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	outreg2 using clean/tables/remit, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over $clust)
	xi: reg pi_hhremitt_p_ch  h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if pi_hhremitt_p_ch<7000 & pi_hhremitt_p_ch>-7000 & hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	outreg2 using clean/tables/remit, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addtext(Treated Area, Over $clust)
...
** WITH INTERACTION
	xi: reg exp1_ch1 h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum exp1, detail
	...
	xi: reg exp1_p_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum exp1_p, detail
	...
	xi: reg food_ch1 h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum food, detail
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg food_p_ch h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum food_p, detail
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)

	xi: reg non_food_1_ch1 h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum non_food_1, detail
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg non_food_1_p_ch  h_ch h_ch_large *_LARGE  m_ch lo lo_large m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum non_food_1_p, detail
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
