...
program define house_quality
	* fixed
	...
	xi: reg wath_ch h_ch h_ch_large large m_ch m_ch_large rdp_r1m h_chL h_chS h_chML i.r*i.mdb if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg toih_ch h_ch h_ch_large large m_ch m_ch_large rdp_r1m h_chL h_chS h_chML  i.r*i.mdb if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg walls_b_ch h_ch h_ch_large large m_ch m_ch_large rdp_r1m h_chL h_chS h_chML   i.r*i.mdb if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg refuse_ch h_ch h_ch_large large m_ch m_ch_large rdp_r1m h_chL h_chS h_chML  i.r*i.mdb if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg mktv_ch h_ch h_ch_large large m_ch m_ch_large  rdp_r1m h_chL h_chS h_chML i.r*i.mdb if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg rooms_ch h_ch h_ch_large large m_ch m_ch_large rdp_r1m h_chL h_chS h_chML  i.r*i.mdb if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over 5)
...
program define health_overall 
	...
	xi: reg zhfa_ch h_ch m_ch a sex rdp_r1m h_chL h_chS h_chML i.r*i.mdb zhfa_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.3 & size_lag>2, robust cluster(hh1)	
	...
	xi: reg zwfa_ch  h_ch m_ch a sex rdp_r1m h_chL h_chS h_chML i.r*i.mdb i.r zwfa_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.66 & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	

	xi: reg c_ill_ch h_ch m_ch a sex rdp_r1m h_chL h_chS h_chML i.r*i.mdb  i.r c_ill_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	
	xi: reg c_health_ch h_ch m_ch a sex rdp_r1m h_chL h_chS h_chML i.r*i.mdb  i.r c_health_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	
...
*	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	
	...
	xi: reg zhfa_ch h_ch__* m_ch__* wath_lag toih_lag h_ch a sex rdp_r1m h_chL h_chS h_chML i.r*i.mdb  zhfa_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s &  zhfa_ch<2 & zhfa_ch>-1.3 & size_lag>2, robust cluster(hh1)	
	...
	xi: reg zwfa_ch  h_ch__* m_ch__* wath_lag toih_lag  h_ch  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  zwfa_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.66  & size_lag>2, robust cluster(hh1)	
	...
	xi: reg c_ill_ch  h_ch__* m_ch__* wath_lag toih_lag  h_ch  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  zwfa_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s  & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall_w_t, nonotes tex(frag) sortvar(h_ch) keep(h_ch__* h_ch wath_lag toih_lag ) label append nocons  addtext(Treated Area, Over $clust)	
	xi: reg c_health_ch  h_ch__* m_ch__* wath_lag toih_lag  h_ch rdp_r1m h_chL h_chS h_chML i.r*i.mdb  zwfa_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s  & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall_w_t, nonotes tex(frag) sortvar(h_ch) keep(h_ch__* h_ch wath_lag toih_lag ) label append nocons  addtext(Treated Area, Over $clust)	
...
program define first_stage
	...
	* FIRST STAGE *
	...
	xi: reg size_ch H_* sl_* mm_*  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if im<=3500 & size_lag<$s  & hclust>=$clust, robust cluster(hh1)
	coefplot, vertical keep(H_*)
	graph export clean/tables/size_ch.pdf, as(pdf) replace

	xi: reg ct_ch H_* sl_* mm_*  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if im<=3500 & size_lag<$s  & hclust>=$clust, robust cluster(hh1)
	coefplot, vertical keep(H_*)
	graph export graphs/child_ch.pdf, as(pdf) replace

	xi: reg ad_ch H_* sl_* mm_*  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if im<=3500 & size_lag<$s  & hclust>=$clust, robust cluster(hh1)
	coefplot, vertical keep(H_*)
	graph export graphs/adult_ch.pdf, as(pdf) replace

	xi: reg crowd_ch H_* sl_* mm_*  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if im<=3500 & size_lag<$s  & hclust>=$clust, robust cluster(hh1)
	coefplot, vertical keep(H_*)
...
*	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch_small h_ch_large small) label append nocons  addtext(Treated Area, Over 5)	
	...
	xi: reg size_ch h_ch h_ch_large  m_ch m_ch_large large  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if  hclust>=0 & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
	...
	xi: reg size_ch h_ch h_ch_large  m_ch m_ch_large large  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
	...
	xi: reg size_ch h_ch h_ch_large  m_ch m_ch_large large  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if  hclust>=10 & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
	...
	xi: reg crowd_ch h_ch h_ch_large  m_ch m_ch_large large  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg ad_ch h_ch h_ch_large  m_ch m_ch_large large  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg ct_ch h_ch h_ch_large m_ch m_ch_large large  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
...
program define reduced_form
	* REDUCED FORM *
	...
	foreach var of varlist rdp_r1m h_chL h_chS h_chML {
	...
	replace h_ch=. if r==1
			...
			* HEIGHT MEASUREMENTS
	xi: reg zhfa_ch $controls a sex  rdp_r1m h_chL h_chS h_chML *_LARGE i.rl*i.mdb  zhfa_p*  if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & zhfa_ch<2 & zhfa_ch>-2.38  & size_lag>2, robust cluster(hh1)	
	coefplot, vertical keep ($graphs) ytitle("Height Change Z-Score")
	graph export clean/tables/height_ch.pdf, as(pdf) replace
		
			* WEIGHT MEASUREMENTS
	xi: reg zwfa_ch $controls a sex  rdp_r1m h_chL h_chS h_chML *_LARGE i.rl*i.mdb zwfa_p*  if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & zwfa_ch<2.47 & zwfa_ch>-2.74 & size_lag>2, robust cluster(hh1)	
...
*	drop if h_ch==1 & own==0
	...
	xi: reg zhfa_ch h_ch h_ch_large m_ch m_ch_large large a sex  rdp_r1m h_chL h_chS h_chML *_LARGE  i.rl*i.mdb  zhfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zhfa_ch<2 & zhfa_ch>-2.38  & size_lag>2, robust cluster(cluster)	
	...
	xi: reg zwfa_ch h_ch h_ch_large m_ch m_ch_large large  a sex  rdp_r1m h_chL h_chS h_chML *_LARGE i.rl*i.mdb  zwfa_p*  if hclust>=$clust & im<=$im & a<$a & size_lag<$s & zwfa_ch<2.47 & zwfa_ch>-2.74 & size_lag>2, robust cluster(cluster)	
...
*	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg c_health_ch h_ch h_ch_large m_ch m_ch_large large  a sex c_health_p*  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(hh1)	
	quietly sum c_health, detail
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	 addstat(Mean, r(mean)) 
	xi: reg c_ill_ch h_ch h_ch_large m_ch m_ch_large large a sex c_ill_p*  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(hh1)	
	quietly sum c_ill, detail
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)  addstat(Mean, r(mean)) 
	
		** APPENDIX ** 
	xi: reg zhfa_ch $controls a sex  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  zhfa_p*  if  (zhfa_p25==0 & zhfa_p50==0) & hclust>=$clust & im<=3500 & a<10 & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.2  & size_lag>2, robust cluster(hh1)	
	coefplot, vertical keep ($graphs)
	xi: reg zhfa_ch $controls  a sex  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  zhfa_p*  if  (zhfa_p25==1 | zhfa_p50==1) &  hclust>=$clust & im<=3500 & a<10 & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.2  & size_lag>2, robust cluster(hh1)	
	coefplot, vertical keep ($graphs)
				* gains are consistent across previously malnourished kids (unfortunately )
	xi: reg zwfa_ch  $controls  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  zwfa_p*  if (zwfa_p25==0 & zwfa_p50==0) & hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zwfa_ch<1.4 & zwfa_ch>-1.6 & size_lag>2, robust cluster(hh1)	
	coefplot, vertical keep ($graphs)
	xi: reg zwfa_ch  $controls  rdp_r1m h_chL h_chS h_chML i.r*i.mdb  zwfa_p*  if (zwfa_p25==1 | zwfa_p50==1) & hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zwfa_ch<1.4 & zwfa_ch>-1.6 & size_lag>2, robust cluster(hh1)	
	coefplot, vertical keep ($graphs)
