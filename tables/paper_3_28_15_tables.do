...
program define parallel_trends
	...
	* height_lag weight_lag size_lag
	...
	xi: reg h_ch zhfa_ch_2 zhfa_lag i.r if im<=$im & a<$a1 & size_lag<$s  & hclust>=$clust, cluster(hh1) robust
	...
	xi: reg h_ch zwfa_ch_2 zwfa_lag i.r if im<=$im & a<$a1 & size_lag<$s  & hclust>=$clust, cluster(hh1) robust
	...
	xi: reg h_ch size_lag rooms_lag inc_ln_lag wath_lag toih_lag i.r if im<=$im & a<100 & size_lag<$s  & hclust>=$clust, cluster(hh1) robust 
...
program define health_overall 
	...
	xi: reg zhfa_ch h_ch m_ch a sex i.r zhfa_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & zhfa_ch<2 & zhfa_ch>-2 & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label replace nocons  addtext(Treated Area, Over 5)	
	xi: reg zhfa_ch h_ch__* m_ch__* wath_lag toih_lag h_ch a sex i.r zhfa_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & zhfa_ch<2 & zhfa_ch>-2 & size_lag>2, robust cluster(hh1)	
	...
	xi: reg zwfa_ch  h_ch m_ch i.r zwfa_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.5 & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg zwfa_ch  h_ch__* m_ch__* wath_lag toih_lag  h_ch i.r zwfa_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.5 & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch__* h_ch wath_lag toih_lag ) label append nocons  addtext(Treated Area, Over 5)	

	xi: reg zbmi_ch  h_ch m_ch i.r zbmi_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zbmi_ch<2.5 & zbmi_ch>-2.5 & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg zbmi_ch  h_ch__* m_ch__* wath_lag toih_lag  h_ch i.r zbmi_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zbmi_ch<2.5 & zbmi_ch>-2.5 & size_lag>2, robust cluster(hh1)	
...
program define reduced_form
	* REDUCED FORM *
	...
	global graphs "H_1 H_2 H_3 H_4 H_5 H_6 H_7 H_8 H_9 HH"
			...
			* HEIGHT MEASUREMENTS
	xi: reg zhfa_ch $controls a sex i.r zhfa_p*  if  hclust>=$clust & im<=3500 & a<10 & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.3  & size_lag>1, robust cluster(hh1)	
	coefplot, vertical keep ($graphs)
	graph export clean/tables/height_ch.pdf, as(pdf) replace
		
			* WEIGHT MEASUREMENTS
	xi: reg zwfa_ch $controls a sex  i.r zwfa_p*  if  hclust>=$clust & im<=3500 & a<13 & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.66 & size_lag>2.32, robust cluster(hh1)	
	coefplot, vertical keep ($graphs)
	graph export clean/tables/weight_ch.pdf, as(pdf) replace
	
			* BMI MEASUREMENTS
	xi: reg zbmi_ch $controls a sex  i.r zbmi_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s &  zbmi_ch<3.4 & zbmi_ch>-2.4 & size_lag>2, robust cluster(hh1)	
	coefplot, vertical keep ($graphs)
	...
	xi: reg zhfa_ch h_ch h_ch_large  m_ch m_ch_large large a sex i.r zhfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.3 & size_lag>2.4, robust cluster(hh1)	
	...
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg c_health_ch h_ch h_ch_large  m_ch m_ch_large large  a sex i.r if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg c_ill_ch h_ch h_ch_large  m_ch m_ch_large large a sex i.r if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)
