...
** GET RID OF CROWDING < IT REALLY DOESN'T WORK ( SHOW AS A ROBUSTNESS CHECK ) >
...
program define summary_stats
	...
	keep if hclust>=5
	keep rdp zhfa zwfa zbmi c_ill c_health size child adult old  crowd rooms wath toih mktv roof_cor walls_b inc health_exp sch_spending food ex
	order rdp zhfa zwfa zbmi c_ill c_health size child adult old  crowd rooms wath toih mktv roof_cor walls_b inc health_exp sch_spending food ex
	rename rdp RDP
...
program define parallel_trends
	...
	* height_lag weight_lag size_lag
	...
	xi: reg h_ch zhfa_ch_2 i.r if im<=$im & a<$a1 & size_lag<$s & hclust>=5, cluster(hh1) robust
	outreg2 using clean/tables/height_weight_lag, nonotes tex(frag) keep(zhfa_ch_2) label replace nocons
	xi: reg h_ch zhfa_ch_2 zhfa_lag i.r if im<=$im & a<$a1 & size_lag<$s  & hclust>=5, cluster(hh1) robust
	outreg2 using clean/tables/height_weight_lag, nonotes tex(frag) keep(zhfa_ch_2 zhfa_lag) label append nocons

	xi: reg h_ch zwfa_ch_2 i.r if im<=$im & a<$a1 & size_lag<$s  & hclust>=5, cluster(hh1) robust
	outreg2 using clean/tables/height_weight_lag, nonotes tex(frag) keep(zwfa_ch_2) label append nocons
	xi: reg h_ch zwfa_ch_2 zwfa_lag i.r if im<=$im & a<$a1 & size_lag<$s  & hclust>=5, cluster(hh1) robust
	outreg2 using clean/tables/height_weight_lag, nonotes tex(frag) keep(zwfa_ch_2 zwfa_lag) label append nocons
	...
	xi: reg h_ch size_lag rooms_lag inc_ln_lag wath_lag toih_lag i.r if im<=$im & a<100 & size_lag<$s  & hclust>=5, cluster(hh1) robust 
	outreg2 using clean/tables/size_lag, nonotes tex(frag) sortvar( size_lag rooms_lag) keep( size_lag  inc_ln_lag wath_lag toih_lag rooms_lag) label replace nocons addnote("All explanatory variables reflect levels in t-1")

	xi: reg h_ch i.size_lag rooms_lag  inc_ln_lag wath_lag toih_lag i.r if im<=$im & a<100 & size_lag<$s & h_chn!=1 & hclust>=5, cluster(hh1) robust 
*	outreg2 using clean/tables/size_lag, nonotes tex(frag) sortvar(size_lag rooms_lag) keep(size_lag  inc_ln_lag wath_lag toih_lag rooms_lag) label append nocons addnote("All explanatory variables reflect levels in t-1")
	...
	xi: reg h_ch size_lag size_ch_2 inc_ln_lag wath_lag toih_lag i.r if im<=$im & a<100 & size_lag<$s & hclust>=5, cluster(hh1) robust 
...
program define house_quality
	...
	xi: reg wath_ch h_ch h_ch_sl size_lag m_ch m_ch_sl i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	...
	xi: reg toih_ch h_ch h_ch_sl size_lag m_ch m_ch_sl i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	...
	xi: reg walls_b_ch h_ch h_ch_sl size_lag m_ch m_ch_sl  i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_sl size_lag ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg rooms_ch h_ch h_ch_sl size_lag m_ch m_ch_sl i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_sl size_lag ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg mktv_ch h_ch h_ch_sl size_lag m_ch m_ch_sl i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_sl size_lag ) label append nocons  addtext(Treated Area, Over 5)



	xi: reg wath_ch H_* sl_* mm_* i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	coefplot, vertical keep(H_*)	
	xi: reg toih_ch H_* sl_* mm_* i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	coefplot, vertical keep(H_*)	
	xi: reg walls_b_ch h_ch H_* sl_* mm_* i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	coefplot, vertical keep(H_*)	
	xi: reg rooms_ch H_* sl_* mm_* i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	coefplot, vertical keep(H_*)	

	xi: reg size_ch H_* sl_* mm_* i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	coefplot, vertical keep(H_*)
	
		** LEVELS
	xi: reg wath h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd  i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	...
	xi: reg toih h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd  i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	...
	xi: reg walls_b h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd  i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/house_quality_levels, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg rooms h_ch h_ch_crowd crowd_lag  m_ch m_ch_crowd i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/house_quality_levels, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
...
program define health_overall 
	...
	xi: reg zhfa_ch h_ch m_ch a sex i.r zhfa_p*  if  hclust>=5 & im<=3500 & a<$s & size_lag<$s & zhfa_ch<2 & zhfa_ch>-2 & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label replace nocons  addtext(Treated Area, Over 5)	
	xi: reg zhfa_ch h_ch__* m_ch__* wath_lag toih_lag h_ch a sex i.r zhfa_p*  if  hclust>=5 & im<=3500 & a<$s & size_lag<$s & zhfa_ch<2 & zhfa_ch>-2 & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch__* h_ch wath_lag toih_lag ) label append nocons  addtext(Treated Area, Over 5)	

	xi: reg zwfa_ch  h_ch m_ch i.r zwfa_p*  if  hclust>=5 & im<=3500 & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.5 & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg zwfa_ch  h_ch__* m_ch__* wath_lag toih_lag  h_ch i.r zwfa_p*  if  hclust>=5 & im<=3500 & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.5 & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch__* h_ch wath_lag toih_lag ) label append nocons  addtext(Treated Area, Over 5)	

	xi: reg zbmi_ch  h_ch m_ch i.r zbmi_p*  if  hclust>=5 & im<=3500 & a<$a & size_lag<$s & zbmi_ch<2.5 & zbmi_ch>-2.5 & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg zbmi_ch  h_ch__* m_ch__* wath_lag toih_lag  h_ch i.r zbmi_p*  if  hclust>=5 & im<=3500 & a<$a & size_lag<$s & zbmi_ch<2.5 & zbmi_ch>-2.5 & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) sortvar(h_ch h_ch__* wath_lag toih_lag) keep(h_ch__* h_ch wath_lag toih_lag ) label append nocons  addtext(Treated Area, Over 5)	
...
program define analysis
	...
	* FIRST STAGE *
	xi: reg size_ch H_* sl_* mm_* i.r if im<=3500 & size_lag<12  & hclust>=5, robust cluster(hh1)
	coefplot, vertical keep(H_*)
	graph export clean/tables/size_ch.pdf, as(pdf) replace

	xi: reg ct_ch H_* sl_* mm_* i.r if im<=3500 & size_lag<12  & hclust>=5, robust cluster(hh1)
	coefplot, vertical keep(H_*)
	graph export graphs/child_ch.pdf, as(pdf) replace

	xi: reg ad_ch H_* sl_* mm_* i.r if im<=3500 & size_lag<12  & hclust>=5, robust cluster(hh1)
	coefplot, vertical keep(H_*)
	graph export graphs/adult_ch.pdf, as(pdf) replace

	xi: reg crowd_ch H_* sl_* mm_* i.r if im<=3500 & size_lag<12  & hclust>=5, robust cluster(hh1)
	coefplot, vertical keep(H_*)
	graph export graphs/crowd_ch.pdf, as(pdf) replace
	
*	xi: reg size_ch *_small *_large small i.r if  hclust>=5 & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
...
*	xi: reg ad_ch *_small *_large small i.r if  hclust>=5 & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
...
*	xi: reg ct_ch *_small *_large small i.r if  hclust>=5 & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
*	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch_small h_ch_large small) label append nocons  addtext(Treated Area, Over 5)	
	...
	xi: reg size_ch h_ch h_ch_large  m_ch m_ch_large large i.r if  hclust>=5 & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
	...
	xi: reg ad_ch h_ch h_ch_large  m_ch m_ch_large large i.r if  hclust>=5 & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg ct_ch h_ch h_ch_large m_ch m_ch_large large i.r if  hclust>=5 & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	

	xi: reg o_ch *_small *_large small size_lag i.r if  hclust>=5 & im<=$im & a<100 & size_lag<12, robust cluster(hh1)	
	...
	* REDUCED FORM *
	...
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg c_health_ch h_ch h_ch_large  m_ch m_ch_large large  a sex i.r if  hclust>=5 & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg c_ill_ch h_ch h_ch_large  m_ch m_ch_large large a sex i.r if  hclust>=5 & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(hh1)	
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
...
program define rooms_analysis
	...
	global s "10"
	global im "3500"
		* there's no kids in one room:: what to do about this?!
	
	* sample size too small: but still worth showing families adjusting size in response to 
	* the new space?!
	
	xi: reg rooms_ch R_* RH_* RM_* a sex i.r  if  hclust>=5 & im<=3500 & a<100 & size_lag<12, robust cluster(hh1)	
	coefplot, vertical keep (RH_*)
	xi: reg size_ch R_* RH_* RM_* a sex i.r  if  hclust>=5 & im<=3500 & a<100 & size_lag<12, robust cluster(hh1)	
	coefplot, vertical keep (RH_*)
	
	tab rooms_lag h_ch if hclust>=5 & im<=3500 & a<10 & size_lag<$s & zhfa_ch<2 & zhfa_ch>-2 & size_lag>2
		
	xi: reg zhfa_ch R_* RH_* RM_* a sex i.r zhfa_p*  if  hclust>=5 & im<=3500 & a<10 & size_lag<$s & zhfa_ch<2 & zhfa_ch>-2 & size_lag>2, robust cluster(hh1)	
	coefplot, vertical keep (RH_*)
	xi: reg zwfa_ch R_* RH_* RM_* a sex i.r zwfa_p*  if  hclust>=5 & im<=3500 & a<10 & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.5 & size_lag>2, robust cluster(hh1)	
	coefplot, vertical keep (RH_*)
		* no trend either way
end
...
program define first_stage_crowding
	use clean/data_analysis/regs_nate_tables_3_6, clear
	label_variables
	global a1 "100"
	global s "12"
	global im "3500"
	la var rooms_ch "Rooms Ch"
	xi: reg crowd_ch h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd  i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/crowd_1, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label replace nocons  addtext(Treated Area, Over 5)
	xi: reg size_ch h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd  i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s & crowd_ch!=., robust cluster(hh1)
	outreg2 using clean/tables/crowd_1, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg rooms_ch h_ch h_ch_crowd crowd_lag  m_ch m_ch_crowd i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s & crowd_ch!=., robust cluster(hh1)
	outreg2 using clean/tables/crowd_1, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg crowd_ch CH_* C_* CM_* i.r if im<=3500 & a<100 & hclust>=5, robust cluster(hh1)
	lab var CH_1 "C 1"
	lab var CH_2 "C 2"
	lab var CH_3 "C 3"
	lab var CH_4 "C 4"
	lab var CH_5 "C 5"
	coefplot, vertical keep(CH_*) label
	graph export clean/tables/crowd1.pdf, replace as(pdf)
end


program define reduced_form_crowding
	...
	xi: reg zwfa_ch h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd a sex i.r zwfa_p*  if hclust>=5 & a<10 & zwfa_ch<2.5 & zwfa_ch>-2.5 & size_lag<$s, robust cluster(hh1)
	xi: reg zhfa_ch h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd a sex i.r zhfa_p*  if hclust>=5 & a<10 & zhfa_ch<2.5 & zhfa_ch>-2.5 & size_lag<$s, robust cluster(hh1)
	xi: reg zhfa_ch h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd a sex i.r zhfa_p*  if hclust>=5 & a<10 & zhfa_ch<2.5 & zhfa_ch>-2.5 & size_lag<$s, robust cluster(hh1)

		hist crowd if hclust>=5 & im<=3500 & a<10 & zwfa_ch<2.5 & zwfa_ch>-2.5, by(h_ch)
	tab crowd h_ch if hclust>=5 & im<=3500 & a<10 & zwfa_ch<2.5 & zwfa_ch>-2.5
	tab crowd h_ch if hclust>=5 & im<=5000 & zwfa_ch!=.
	
	xi: reg zwfa_ch CH_* C_* CM_* a sex  i.r zwfa_p*  if hclust>=5 & im<=5000 & zwfa_ch!=., robust cluster(hh1)	
	...
	xi: reg zwfh_ch CH_* C_* CM_*  a sex i.r zwfh_p*  if hclust>=5 & im<=3500 & a<10 & zwfh_ch<2.5 & zwfh_ch>-2.5, robust cluster(hh1)	
	coefplot, vertical keep (CH_*)
	xi: reg zbmi_ch CH_* C_* CM_*  a sex i.r zbmi_p*  if hclust>=5 & im<=3500 & a<10 & zbmi_ch<4 & zbmi_ch>-4, robust cluster(hh1)	
	coefplot, vertical keep (CH_*)
...
*	replace h_ch=. if h_chn==1
	...
	sort pid r
	foreach var of varlist zwfa zhfa zbmi zwfh {
