...
program define adult_weight_1
	...
	forvalues r=10(5)70 {
	g ad_`r'=(a>=`r'-5 & a<`r')
	g h_ch_ad_`r'=h_ch*ad_`r'
	g ad_sl_`r'=ad_`r'*size_lag
	g h_ch_sl_ad_`r'=h_ch*ad_`r'*size_lag
	}
	g h_ch_sl=h_ch*size_lag
	
*	xi: reg zw_ch h_ch h_ch_ad_* h_ch_sl h_ch_sl_ad*  ad_*  size_lag  sex i.r if a<70 & size_lag<11, robust cluster(hh1)

*	g at_7=(a>10)
*	g h_ch_at_7=h_ch*at_7
*	g at_sl_7=at_7*size_lag
*	g h_ch_sl_at_7=h_ch*at_7*size_lag
...
*	g at_10_20=(a>10 & a<=20)
*	g h_ch_at_10_20=h_ch*at_10_20
*	g at_sl_10_20=at_10_20*size_lag
*	g h_ch_sl_at_10_20=h_ch*at_10_20*size_lag
...
*	g at_80=(a>60)
*	g h_ch_at_80=h_ch*at_80
*	g at_sl_80=at_80*size_lag
*	g h_ch_sl_at_80=h_ch*at_80*size_lag
	...
	by pid: g f_res_ch=f_res[_n]-f_res[_n-1]
		...
		* not an issue of parents residing with kids!
	xi: reg m_res_ch h_ch h_ch_sl size_lag a sex i.r if size_lag<=11 & a<=20, robust cluster(hh1)
	xi: reg f_res_ch h_ch h_ch_sl size_lag a sex i.r if size_lag<=11 & a<=20, robust cluster(hh1)
...
*	forvalues r=1/16 {
*	sort hhid
*	by hhid: g fpid`r'=f_pid[`r']
*	by hhid: g mpid`r'=m_pid[`r']
*	egen fpm`r'=max(fpid`r'), by(hhid)
*	egen mpm`r'=max(mpid`r'), by(hhid)
*	replace parent=1 if pid==fpm`r' | pid==mpm`r'
*	drop fpid`r' fpm`r' mpid`r' mpm`r'
*	}
	** too much error in this measure!

	

	xi: reg zw_ch h_ch h_ch_at_* h_ch_sl h_ch_sl_at*  at_* size_lag a sex i.r if size_lag<=11 & a<50, robust cluster(hh1)
	...
	xi: reg zw_ch h_ch h_ch_at_* h_ch_sl h_ch_sl_at*  at_* size_lag a i.r if size_lag<=11 & a<70, robust cluster(hh1)
	xi: reg zw_ch h_ch h_ch_at_* h_ch_sl h_ch_sl_at*  at_* size_lag a i.r if size_lag<=11 & a<70, robust cluster(hh1)
...
** WEAKER IF A PARENT IS RESIDENT?
	xi: reg zw_ch i.h_ch*i.m_res i.m_res*h_ch_sl i.m_res*size_lag a sex i.r if size_lag<=11 & a<=7, robust cluster(hh1)
		* stronger if a parent is not resident
	xi: reg zw_ch i.h_ch*i.f_res i.f_res*h_ch_sl i.f_res*size_lag a sex i.r if size_lag<=11 & a<=7, robust cluster(hh1)


** ** **
	xi: reg zw_ch i.h_ch*i.child_res i.child_res*h_ch_sl i.child_res*size_lag a sex i.r if size_lag<=11 & a>18 & a<60, robust cluster(hh1)

	xi: reg zw_ch i.h_ch*i.child_res i.child_res*h_ch_sl i.child_res*size_lag a sex i.r if size_lag<=11 & a>18 & a<60 & sex==0, robust cluster(hh1)



	xi: reg zw_ch i.h_ch*i.child_res i.child_res*size_lag a sex i.r if size_lag<=11 & a>18 & a<60, robust cluster(hh1)

** biological child resident! then you don't do better!!! THIS STUFF IS ACTUALLY HAPPENING
	...
	xi: reg zw_ch i.h_ch*i.child_res i.child_res*h_ch_sl i.child_res*size_lag a sex i.r if size_lag<=11 & a>18 & a<50, robust cluster(hh1)
		...
		* driven by parents also?
	xi: reg zw_ch i.h_ch*i.parent i.parent*h_ch_sl i.parent*size_lag a sex i.r if size_lag<=11 & a>18 & a<60, robust cluster(hh1)

	xi: reg zw_ch i.h_ch*i.parent i.parent*h_ch_sl i.parent*size_lag a sex i.r if size_lag<=11 & a>18 & a<60 & sex==1, robust cluster(hh1)
	xi: reg zw_ch i.h_ch*i.parent i.parent*h_ch_sl i.parent*size_lag a sex i.r if size_lag<=11 & a>18 & a<60 & sex==0, robust cluster(hh1)
	...
	xi: reg size_ch h_ch h_ch_sl size_lag a sex i.r if size_lag<=11 & a<60, robust cluster(hh1)
