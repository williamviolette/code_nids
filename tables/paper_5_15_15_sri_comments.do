...
program define left_out_descriptives
	...
	lab var cres "Number of Children (Mothers Only)"
	lab var hoh "Head of Household"
	lab var a  "Age"
	lab var sex "Gender (1 for male)"
	lab var e "Employed"
	
	keep Left_Out 	a sex e cres hoh 
	order Left_Out 	 a sex e cres hoh
	bysort Left_Out: outreg2 using clean/tables/sum_left_out, noni sum(log) eqkeep(mean N) label tex(frag) replace title("Summary Statistics: Left-Out versus RDP Beneficiaries")

	estpost ttest 	a sex e cres hoh , by(Left_Out)
	est sto t1
...
program define left_out
	...
	outreg2 using clean/tables/left_out, nonotes tex(frag) keep(lo lo_large  large ) label replace sortvar(lo lo_large lo2 lo2_large large) nocons  addtext(Treated Area, Over 5)
	...
	outreg2 using clean/tables/left_out, nonotes tex(frag) keep(lo lo_large  large) label append sortvar(lo lo_large lo2 lo2_large large) nocons  addtext(Treated Area, Over 5)
...
*egen max_ml=max(h_chML), by(pid)
*drop if max_ml==1
	replace rooms_lag=. if rooms_lag>8
	sort pid r
	by pid: g im_lag=im[_n-1]
	label variable im_lag "Income Per t-1"
	
	global clust "0"
	xi: reg zhfa_ch h_ch h_ch_large  h_chS h_chS_large h_chML_large  m_ch  m_ch_large large a sex  h_chML im_lag rooms_lag zhfa_p*  i.r*i.prov if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.5 & size_lag>2, robust cluster(cluster)	
	quietly sum zhfa, detail
	scalar define mean1=r(mean)
	quietly	test _b[h_ch]+_b[h_ch_large]=0
	outreg2 using clean/tables/health_appendix, nonotes tex(frag) sortvar(h_ch h_ch_large h_chS h_chS_large large lo lo_large im_lag rooms_lag) keep(h_ch h_ch_large h_chS h_chS_large large lo lo_large im_lag rooms_lag) label replace  nocons  addtext(Time x Prov FE, YES, Time x Prov x Large FE, NO, Treated Area, Full Sample, Rooms t-1 and Inc Per t-1, YES)  addstat(Mean, mean1, F-Stat: RDP+RDPxLarge=0, r(F) ) 	addnote("All regressions control for lagged quartiles in outcomes")	

	scalar drop mean1
	global clust "0"
	xi: reg zhfa_ch h_ch h_ch_large  h_chS h_chS_large h_chML_large  m_ch  m_ch_large a sex  h_chML im_lag rooms_lag i.rl1*i.prov zhfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.5 & size_lag>2, robust cluster(cluster)	
	quietly sum zhfa, detail
	scalar define mean1=r(mean)
	quietly	test _b[h_ch]+_b[h_ch_large]=0
	outreg2 using clean/tables/health_appendix, nonotes tex(frag) sortvar(h_ch h_ch_large h_chS h_chS_large large lo lo_large im_lag rooms_lag) keep(h_ch h_ch_large h_chS h_chS_large large lo lo_large im_lag rooms_lag) label append  nocons  addtext(Time x Prov FE, NO, Time x Prov x Large FE, YES, Treated Area, Full Sample, Rooms t-1 and Inc Per t-1, YES)  addstat(Mean, mean1, F-Stat: RDP+RDPxLarge=0, r(F) ) 	addnote("All regressions control for lagged quartiles in outcomes")	

	scalar drop mean1
	global clust "5"
	xi: reg zhfa_ch h_ch h_ch_large  h_chS_large h_chML_large  m_ch m_ch_large a sex h_chS h_chML im_lag rooms_lag i.rl1*i.prov  zhfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.5 & size_lag>2, robust cluster(cluster)	
	quietly sum zhfa, detail
	scalar define mean1=r(mean)
	quietly	test _b[h_ch]+_b[h_ch_large]=0	
	outreg2 using clean/tables/health_appendix, nonotes tex(frag) keep(h_ch h_ch_large  h_chS h_chS_large lo lo_large im_lag rooms_lag) label append nocons  addtext(Time x Prov FE, NO, Time x Prov x Large FE, YES, Treated Area, Over 5, Rooms t-1 and Inc Per t-1, YES) addstat(Mean, mean1, F-Stat: RDP+RDPxLarge=0, r(F) ) 	addnote("All regressions control for lagged quartiles in outcomes")	

	scalar drop mean1
	global clust "0"
	xi: reg zwfa_ch h_ch h_ch_large  h_chS_large h_chML_large  m_ch  m_ch_large large  a sex  h_chS h_chML  zwfa_p* im_lag rooms_lag i.r*i.prov if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.66 & size_lag>2, robust cluster(cluster)	
	quietly sum zwfa, detail
	scalar define mean1=r(mean)
	quietly	test _b[h_ch]+_b[h_ch_large]=0	
	outreg2 using clean/tables/health_appendix, nonotes tex(frag) keep(h_ch h_ch_large large h_chS h_chS_large lo lo_large im_lag rooms_lag) label append nocons   addtext(Time x Prov FE, YES, Time x Prov x Large FE, NO, Treated Area, Full Sample, Rooms t-1 and Inc Per t-1, YES)  addstat(Mean, mean1, F-Stat: RDP+RDPxLarge=0, r(F) )	addnote("All regressions control for lagged quartiles in outcomes")	

	scalar drop mean1
	global clust "0"
	xi: reg zwfa_ch h_ch h_ch_large  h_chS_large h_chML_large  m_ch m_ch_large  a sex  h_chS h_chML im_lag rooms_lag i.rl1*i.prov  zwfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.66 & size_lag>2, robust cluster(cluster)	
	quietly sum zwfa, detail
	scalar define mean1=r(mean)
	quietly	test _b[h_ch]+_b[h_ch_large]=0	
	outreg2 using clean/tables/health_appendix, nonotes tex(frag) keep(h_ch h_ch_large large h_chS h_chS_large lo lo_large im_lag rooms_lag) label append nocons  addtext(Time x Prov FE, NO, Time x Prov x Large FE, YES, Treated Area, Full Sample, Rooms t-1 and Inc Per t-1, YES)  addstat(Mean, mean1, F-Stat: RDP+RDPxLarge=0, r(F) ) 	addnote("All regressions control for lagged quartiles in outcomes")	
...
program define parallel_trends_appendix
	...
	* height_lag weight_lag size_lag
	...
	la var mktv_lag "Market Value"
		* what kind of selection do we find? ( currently turned off size disaggregation )
	xi: reg h_ch size_lag inc_ln_lag avg_e_lag h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s  & hclust>=0, cluster(cluster) robust 
	outreg2 using clean/tables/size_lead_appendix, nonotes tex(frag) sortvar( size_lag ) keep( size_lag  inc_ln_lag avg_e_lag wath_lag toih_lag rooms_lag walls_b_lag own_lag) label replace  nocons addtext(Treated Area, Full Sample, One Person HH, YES) addnote("All explanatory variables reflect levels in t+1")
	xi: reg h_ch size_lag inc_ln_lag avg_e_lag h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s  & hclust>=5, cluster(cluster) robust 
	outreg2 using clean/tables/size_lead_appendix, nonotes tex(frag) sortvar( size_lag ) keep( size_lag  inc_ln_lag avg_e_lag wath_lag toih_lag rooms_lag walls_b_lag own_lag) label append nocons addtext(Treated Area, Over 5, One Person HH, YES) addnote("All explanatory variables reflect levels in t+1")
	xi: reg h_ch size_lag inc_ln_lag avg_e_lag h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s  & hclust>=10, cluster(cluster) robust 
	outreg2 using clean/tables/size_lead_appendix, nonotes tex(frag) sortvar( size_lag ) keep( size_lag  inc_ln_lag avg_e_lag wath_lag toih_lag rooms_lag walls_b_lag own_lag) label append nocons addtext(Treated Area, Over 10, One Person HH, YES) addnote("All explanatory variables reflect levels in t+1")
	xi: reg h_ch size_lag inc_ln_lag avg_e_lag h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s  & hclust>=5 & size_lag>1, cluster(cluster) robust 
	outreg2 using clean/tables/size_lead_appendix, nonotes tex(frag) sortvar( size_lag ) keep( size_lag  inc_ln_lag avg_e_lag wath_lag toih_lag rooms_lag walls_b_lag own_lag) label append nocons addtext(Treated Area, Over 5, One Person HH, YES) addnote("All explanatory variables reflect levels in t+1")	
	xi: reg h_ch size_lag inc_ln_lag avg_e_lag h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s  & hclust>=5, cluster(cluster) robust 
	outreg2 using clean/tables/size_lead_appendix, nonotes tex(frag) sortvar( size_lag ) keep( size_lag  inc_ln_lag avg_e_lag wath_lag toih_lag rooms_lag walls_b_lag own_lag mktv_lag) label append nocons addtext(Treated Area, Over 5, One Person HH, YES) addnote("All explanatory variables reflect levels in t+1")
