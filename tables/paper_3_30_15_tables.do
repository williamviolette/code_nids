...
*	outreg2 using clean/tables/height_weight_lag, nonotes tex(frag) keep(zwfa_ch_2) label append nocons
	...
	xi: reg h_ch size_lag inc_ln_lag avg_e_lag rooms_lag wath_lag toih_lag walls_b_lag own_lag mktv_lag i.r if im<=$im & a<100 & size_lag<$s  & hclust>=10, cluster(hh1) robust 
	outreg2 using clean/tables/size_lag, nonotes tex(frag) sortvar( size_lag ) keep( size_lag  inc_ln_lag avg_e_lag wath_lag toih_lag rooms_lag walls_b_lag own_lag mktv_lag) label append nocons addtext(Treated Area, Over 10) addnote("All explanatory variables reflect levels in t-1")
...
** WITHOUT INTERACTION 
	xi: reg exp1_ch1 h_ch   m_ch  i.r  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s & exp1_ch<2500 & exp1_ch>-2500, robust cluster(hh1)	
	...
	xi: reg exp1_p_ch h_ch   m_ch  i.r  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s &  exp1_p_ch<1000 & exp1_p_ch>-1000, robust cluster(hh1)	
	...
	xi: reg food_ch1 h_ch  m_ch   i.r  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s & food_ch<1500 & food_ch>-1500, robust cluster(hh1)	
	outreg2 using clean/tables/exp, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addtext(Treated Area, Over $clust)
	xi: reg food_p_ch h_ch  m_ch  i.r  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s & food_p_ch<800 & food_p_ch>-800, robust cluster(hh1)	
	outreg2 using clean/tables/exp, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over $clust)

	xi: reg non_food_1_ch1 h_ch  m_ch   i.r  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s & non_food_1_ch<1500 & non_food_1_ch >-1500, robust cluster(hh1)	
	outreg2 using clean/tables/exp, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addtext(Treated Area, Over $clust)
	xi: reg non_food_1_p_ch  h_ch   m_ch   i.r  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s & non_food_1_p_ch<800 & non_food_1_p_ch>-800, robust cluster(hh1)	
	outreg2 using clean/tables/exp, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over $clust)
...
** WITH INTERACTION
	...
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large  ) label replace nocons  addtext(Treated Area, Over $clust)
	...
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over $clust)
	...
	xi: reg non_food_1_ch1 h_ch h_ch_large  m_ch m_ch_large large  i.r  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s & non_food_1_ch<1500 & non_food_1_ch >-1500, robust cluster(hh1)	
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addtext(Treated Area, Over $clust)
	xi: reg non_food_1_p_ch  h_ch h_ch_large  m_ch m_ch_large large  i.r  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s & non_food_1_p_ch<800 & non_food_1_p_ch>-800, robust cluster(hh1)	
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over $clust)
...
*	}
	* fix rent variable
	...
	g non_food_1=non_food+public
	foreach var of varlist non_food_1 food exp1 exp_n {
