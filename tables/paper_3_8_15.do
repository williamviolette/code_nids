...
program define parallel_trends
	...
	* height_lag weight_lag size_lag
	...
	outreg2 using clean/tables/weight_lag, nonotes tex(frag) keep(zwfa_ch_2 zwfa_lag) label append nocons
		...
		* what kind of selection do we find?
...
program define label_variables
	...
	* Change Variables
	...
	label variable wath_ch "Flush Toilet Ch"
	label variable toih_ch "Piped Water Ch"	
...
program define sum_rdp_numbers1
	...
	outreg2 using clean/tables/sum_rdp1, noni sum(log) eqkeep(mean N) label tex(frag) replace title("Subsidized Housing (RDP) in Analysis Sample")
end
