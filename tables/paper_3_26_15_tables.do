...
program define parallel_trends
	...
	* height_lag weight_lag size_lag
	...
	xi: reg h_ch crowd_lag  inc_ln_lag wath_lag toih_lag i.r if im<=$im & a<100 & size_lag<$s  & hclust>=5, cluster(hh1) robust 
	outreg2 using clean/tables/size_lag, nonotes tex(frag) sortvar(crowd_lag size_lag rooms_lag) keep(crowd_lag size_lag  inc_ln_lag wath_lag toih_lag rooms_lag) label replace nocons addnote("All explanatory variables reflect levels in t-1")

	xi: reg h_ch rooms_lag size_lag inc_ln_lag wath_lag toih_lag i.r if im<=$im & a<100 & size_lag<$s & h_chn!=1 & hclust>=5, cluster(hh1) robust 
	outreg2 using clean/tables/size_lag, nonotes tex(frag) sortvar(crowd_lag size_lag rooms_lag) keep(size_lag  inc_ln_lag wath_lag toih_lag rooms_lag) label append nocons addnote("All explanatory variables reflect levels in t-1")

	xi: reg h_ch crowd_lag crowd_ch_2 inc_ln_lag wath_lag toih_lag i.r if im<=$im & a<100 & size_lag<$s & hclust>=5, cluster(hh1) robust 
		* robust to testing selection non-parametrically and to lagged crowding changes
...
program define house_quality
	...
	xi: reg wath_ch h_ch  m_ch  i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label replace nocons  addtext(Treated Area, Over 5)
	xi: reg wath_ch h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg toih_ch h_ch m_ch  i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg toih_ch h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd  i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg walls_b_ch h_ch  m_ch   i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg walls_b_ch h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd  i.r if hclust>=5 & im<=$im & a<$a1 & size_lag<$s, robust cluster(hh1)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
