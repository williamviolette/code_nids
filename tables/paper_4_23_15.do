...
program define left_out1
	...
	egen nc1=max(noch1), by(hhid)
	egen nc2=max(noch2), by(hhid)
	sort pid r	
	by pid: g nr1=(nc1[_n-1]==1 & r==2 & h_ch==1)
	by pid: g nr2=(nc2[_n-1]==1 & r==3 & h_ch==1)
	g nr=(nr1==1 | nr2==1)
	
	egen nc1s=sum(noch1), by(hhid)
	egen nc2s=sum(noch2), by(hhid)
	
	sort pid r
	g ncs=nc1s+nc2s
	by pid: g ns=ncs[_n-1]
	
	tab ns lo
*	hist ns, by (lo)
	sort pid r
	by pid: g h_ch_lead=h_ch[_n+1]
	egen hcl=max(h_ch_lead), by(hhid)
	
	g a_ncs=a*ncs
	
	tab a if lo==1
	
	** HOW OLD ARE THE PEOPLE LEFT OUT BY NUMBER OF PEOPLE LEFT OUT?!
*	hist a if hcl==1 & ncs>0 & noch==1, by(ncs)

	tab a ncs if hcl==1 & ncs>0 & noch==1
	
	xi: reg a ncs i.r if noch==1, robust cluster(cluster)
	
	g mother=(cres>0 & cres<.)
	g mother_age=a if mother==1
	g mother_age50=mother_age if mother_age<50
	g mother50=mother if mother_age<50
	
	egen m50max=max(mother_age50), by(hhid)
	egen m50min=min(mother_age50), by(hhid)
	g mdiff=m50max-m50min
	
	replace mother50=0 if mdiff>12 | mdiff==0

	egen m2=sum(mother50), by(hhid)
	replace m2=1 if mdiff>12 & mdiff<.
	
	g m2id=(m2>=2 & m2<.)
	
	egen crt=sum(cres), by(hhid)
	egen crm=max(cres), by(hhid)
	
	g cratio=crm/crt
	
	**** DOES KIDS RATIO PREDICT WHETHER A HOUSEHOLD SPLITS? **
	sort pid r
	by pid: g schl=size_ch[_n+1]
	g split_ratio=schl/size
	g split=split_ratio<-.2
	
	tab split h_ch_lead
	
	tab size split if h_ch_lead==1

	* Does cratio affect the likelihood of splitting period?
	g m_wratio=adult_men/adult_women
	
	g m_wdiff=abs(adult_men-adult_women)

	* men and women ratio doesn't matter
	xi: reg schl m_wratio i.child i.adult_men i.adult_women i.r if h_ch_lead==1 & hclust>=$clust & im<=$im, robust cluster(cluster)

	* men and women difference sort of matters
	xi: reg schl m_wdiff i.child i.adult_men i.adult_women i.r if h_ch_lead==1 & hclust>=$clust & im<=$im, robust cluster(cluster)

	xi: reg split m_wdiff child i.adult_men i.adult_women i.r if h_ch_lead==1 & hclust>=$clust & im<=$im, robust cluster(cluster)


	xi: reg schl i.m2 i.child i.adult_men i.adult_women i.r if h_ch_lead==1 & hclust>=$clust & im<=$im, robust cluster(cluster)

		****	
	
	* number of children for each mother matters but only with 3 mothers in the hh
		* which is like 2 hh's..
	xi: reg schl i.m2 i.child i.adult_men i.adult_women i.r if h_ch_lead==1 & hclust>=$clust & im<=$im, robust cluster(cluster)

	xi: reg split m2 child adult_men adult_women i.r, robust cluster(cluster)
	
	xi: reg split i.h_ch_lead*child i.h_ch_lead*adult_men i.h_ch_lead*adult_women i.h_ch_lead*i.m2 i.r, robust cluster(cluster)
	

	xi: reg split i.h_ch_lead*child i.h_ch_lead*adult i.r, robust cluster(cluster)

	xi: reg split i.h_ch_lead*child i.h_ch_lead*adult_men i.h_ch_lead*adult_women i.h_ch_lead*m2 i.r, robust cluster(cluster)


	* Does cratio affect the type of split?
	
	xi: reg size_ch_lead cratio size i.m2 i.r if m2id==1, robust cluster(cluster)
	
	
	
	g kid_ratio=child/size

	
*	hist a if hcl==1, by(noch)

	xi: reg split i.h_ch_lead*kid_ratio i.h_ch_lead*avg_e  i.r if  hclust>=$clust & im<=$im & size>3 & size<11, robust cluster(cluster)
	
	xi: reg split h_ch_lead kid_ratio avg_e i.r if  hclust>=$clust & im<=$im & size>3 & size<11, robust cluster(cluster)
	
	xi: reg split i.h_ch_lead*kid_ratio  adult child size i.r if  hclust>=$clust & im<=$im & size>3 & size<11, robust cluster(cluster)
	
	xi: reg noch e sex a i.r if hcl==1 & hclust>=$clust & im<=$im, robust cluster(cluster)
	
	
	hist size, by(noch)
	
	tab size_lag size_ch if hcl==1
	
	hist size_ch if size_lag<=11, by(size_lag h_ch)
	
	
	xi: reg noch e hoh a sex i.r if hcl==1, robust cluster(cluster)
	xi: reg noch a i.sex*ncs a_ncs i.r if hcl==1, robust cluster(cluster)


	xi: reg noch fwag a sex i.r if hcl==1, robust cluster(cluster)

	xi: reg noch a sex c_ill c_health i.r if hcl==1, robust cluster(cluster)

	xi: reg noch a sex a_emobth a_emomnd a_emodep a_emoeff a_emohope a_emofear a_emoslp a_emohap a_emolone  i.r if hcl==1, robust cluster(cluster)

	xi: reg noch a sex c_ill c_health zwfa zhfa i.r if hcl==1, robust cluster(cluster)

		    
	tab size_ch 
	
	
	xi: reg h_ch zwfa_ch_2 zwfa_lag  inc_ln_lag inc_ln_ch_2 h_chS h_chML  *_LARGE i.r*i.prov  if im<=$im & a<$a1 & size_lag<$s  & hclust>=$clust, cluster(cluster) robust
...
*	xi: reg h_ch zhfa_ch_2 h_chS h_chML    i.r*i.prov  if im<=$im & a<$a1 & size_lag<$s & hclust>=$clust, cluster(cluster) robust
...
*	xi: reg h_ch zwfa_ch_2 h_chS h_chML    i.r*i.prov  if im<=$im & a<$a1 & size_lag<$s  & hclust>=$clust, cluster(cluster) robust
*	outreg2 using clean/tables/height_weight_lag, nonotes tex(frag) keep(zwfa_ch_2) label append nocons
	...
	xi: reg h_ch large inc_ln_lag avg_e_lag rooms_lag wath_lag toih_lag walls_b_lag own_lag h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s  & hclust>=5, cluster(cluster) robust 

		* are these interactions important?!
	xi: reg h_ch i.large*inc_ln_lag i.large*avg_e_lag i.large*rooms_lag i.large*wath_lag i.large*toih_lag i.large*walls_b_lag i.large*own_lag h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s  & hclust>=5, cluster(cluster) robust 


	xi: reg h_ch i.size_lag rooms_lag  inc_ln_lag wath_lag toih_lag h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s & h_chn!=1 & hclust>=$clust, cluster(cluster) robust 
*	outreg2 using clean/tables/size_lag, nonotes tex(frag) sortvar(size_lag rooms_lag) keep(size_lag  inc_ln_lag wath_lag toih_lag rooms_lag) label append nocons addnote("All explanatory variables reflect levels in t-1")
	...
	xi: reg h_ch size_lag size_ch_2 inc_ln_lag wath_lag toih_lag h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s & hclust>=$clust, cluster(cluster) robust 
		* robust to testing selection non-parametrically and to lagged size changes

	*** CONFRONT THIS ISSUE LATER ***
*	xi: reg rooms size_lag h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s & rooms<10 & h_ch==1 & hclust>5, cluster(cluster) robust 
*	xi: reg size_lag rooms wath toih h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s & rooms<10 & h_ch==1 & hclust>5, cluster(cluster) robust 
*	xi: reg size_lag rooms_ch wath_ch toih_ch h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s & rooms<10 & h_ch==1, cluster(cluster) robust 
*	xi: reg size_lag rooms wath toih h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s & rooms<10 & h_ch==1, cluster(cluster) robust 
*	xi: reg size_lag rooms_ch wath_ch toih_ch h_chS h_chML    i.r*i.prov  if im<=$im & a<100 & size_lag<$s & rooms<10 & h_ch==1, cluster(cluster) robust 
...
program define house_quality
	...
	xi: reg wath_ch h_ch h_ch_large large m_ch m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg toih_ch h_ch h_ch_large large m_ch m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg walls_b_ch h_ch h_ch_large large m_ch m_ch_large  h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg refuse_ch h_ch h_ch_large large m_ch m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg mktv_ch h_ch h_ch_large large m_ch m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg rooms_ch h_ch h_ch_large large m_ch m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over 5)


** WITHOUT INTERACTIONS
	xi: reg h_freqdomvio_ch h_ch m_ch h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/n_quality, nonotes tex(frag) keep(h_ch ) label replace nocons  addtext(Treated Area, Over 5)
	xi: reg h_freqvio_ch h_ch m_ch  h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/n_quality, nonotes tex(frag) keep(h_ch ) label append nocons  addtext(Treated Area, Over 5)
*	xi: reg h_freqmdr_ch h_ch  m_ch h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
*	outreg2 using clean/tables/n_quality, nonotes tex(frag) keep(h_ch ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg h_freqdrug_ch h_ch  m_ch h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/n_quality, nonotes tex(frag) keep(h_ch ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg n_trust_ch h_ch  m_ch h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/n_quality, nonotes tex(frag) keep(h_ch ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg n_stay_ch h_ch  m_ch h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/n_quality, nonotes tex(frag) keep(h_ch ) label append nocons  addtext(Treated Area, Over 5)

***** INCLUDES INTERACTIONS
	xi: reg h_freqdomvio_ch h_ch h_ch_large large m_ch m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg h_freqvio_ch h_ch h_ch_large large m_ch m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg h_freqdrug_ch h_ch h_ch_large large m_ch m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/n_quality_i, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg n_trust_ch h_ch h_ch_large large m_ch m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/n_quality_i, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg n_stay_ch h_ch h_ch_large large m_ch m_ch_large h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/n_quality_i, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over 5)

	** TEST THAT PERCEPTIONS ARE THE SAME AT BASELINE
	
	xi: reg h_freqdomvio large1 h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s & h_ch_t1==1, robust cluster(cluster)
	outreg2 using clean/tables/n_quality_t, nonotes tex(frag) keep(large1 ) label replace nocons  addtext(Treated Area, Over 5)
	xi: reg h_freqvio large1 h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s & h_ch_t1==1, robust cluster(cluster)
	outreg2 using clean/tables/n_quality_t, nonotes tex(frag) keep(large1 ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg h_freqdrug large1 h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s & h_ch_t1==1, robust cluster(cluster)
	outreg2 using clean/tables/n_quality_t, nonotes tex(frag) keep(large1 ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg n_trust large1 h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s & h_ch_t1==1, robust cluster(cluster)
	outreg2 using clean/tables/n_quality_t, nonotes tex(frag) keep(large1 ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg n_stay large1 h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s & h_ch_t1==1, robust cluster(cluster)
	outreg2 using clean/tables/n_quality_t, nonotes tex(frag) keep(large1 ) label append nocons  addtext(Treated Area, Over 5)


	tab size_lag h_ch if h_freqdomvio_ch!=.

	xi: reg h_freqdomvio_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	coefplot, vertical keep(H_*)	
	xi: reg h_freqvio_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	coefplot, vertical keep(H_*)	
	xi: reg h_freqgang_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	coefplot, vertical keep(H_*)	
	xi: reg h_freqmdr_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	coefplot, vertical keep(H_*)	
	xi: reg h_freqdrug_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	coefplot, vertical keep(H_*)	


	xi: reg wath_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	coefplot, vertical keep(H_*)	
	xi: reg toih_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	coefplot, vertical keep(H_*)	
	xi: reg walls_b_ch h_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	coefplot, vertical keep(H_*)	
	xi: reg rooms_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	coefplot, vertical keep(H_*)	

	xi: reg size_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	coefplot, vertical keep(H_*)
	
		** LEVELS
	xi: reg wath h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd  h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg toih h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd  h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	...
	xi: reg walls_b h_ch h_ch_crowd crowd_lag m_ch m_ch_crowd  h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality_levels, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
	xi: reg rooms h_ch h_ch_crowd crowd_lag  m_ch m_ch_crowd h_chS h_chML    i.r*i.prov  if hclust>=$clust & im<=$im & a<$a1 & size_lag<$s, robust cluster(cluster)
	outreg2 using clean/tables/house_quality_levels, nonotes tex(frag) keep(h_ch h_ch_crowd crowd_lag ) label append nocons  addtext(Treated Area, Over 5)
end
...
program define health_overall 
	...
	xi: reg zhfa_ch h_ch m_ch a sex h_chS h_chML    i.r*i.prov zhfa_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.3 & size_lag>2, robust cluster(cluster)	
	...
	xi: reg zwfa_ch  h_ch m_ch h_chS h_chML    i.r*i.prov zwfa_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.66 & size_lag>2, robust cluster(cluster)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	

	xi: reg c_ill_ch h_ch m_ch a sex h_chS h_chML    i.r*i.prov c_ill_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & size_lag>2, robust cluster(cluster)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	
	xi: reg c_health_ch h_ch m_ch a sex h_chS h_chML    i.r*i.prov c_health_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & size_lag>2, robust cluster(cluster)	
	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	

*	xi: reg c_absent_ch h_ch m_ch a sex h_chS h_chML    i.r*i.prov c_absent_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & size_lag>2, robust cluster(cluster)	
*	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	
*	xi: reg c_failed_ch h_ch m_ch a sex h_chS h_chML    i.r*i.prov c_failed_p*  if  hclust>=$clust & im<=3500 & a<$s & size_lag<$s & size_lag>2, robust cluster(cluster)	
*	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	
...
*	xi: reg zbmi_ch  h_ch m_ch h_chS h_chML    i.r*i.prov zbmi_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zbmi_ch<3.4 & zbmi_ch>-2.4 & size_lag>2, robust cluster(cluster)	
*	outreg2 using clean/tables/health_overall, nonotes tex(frag) keep(h_ch) label append nocons  addtext(Treated Area, Over $clust)	
*	xi: reg zbmi_ch  h_ch__* m_ch__* wath_lag toih_lag  h_ch h_chS h_chML    i.r*i.prov zbmi_p*  if  hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zbmi_ch<3.4 & zbmi_ch>-2.4 & size_lag>2, robust cluster(cluster)	
...
program define first_stage
	...
	* FIRST STAGE *
	...
	xi: reg size_ch adultH_* childH_* childsl_* childmm_* adultsl_* adultmm_* h_chS h_chML    i.r*i.prov  if im<=3500 & adult_lag<7 & child_lag<7 & size_lag<$s  & hclust>=$clust , robust cluster(cluster)
	coefplot, vertical keep(adultH_* childH_*)
		* this is kind of hard to interpret..
	

	xi: reg size_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if im<=3500 & size_lag<$s  & hclust>=$clust, robust cluster(cluster)
	coefplot, vertical keep(H_*)
	graph export clean/tables/size_ch.pdf, as(pdf) replace

	xi: reg ct_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if im<=3500 & size_lag<$s  & hclust>=$clust, robust cluster(cluster)
	coefplot, vertical keep(H_*)
	graph export graphs/child_ch.pdf, as(pdf) replace

	xi: reg ad_ch H_* sl_* mm_* h_chS h_chML    i.r*i.prov  if im<=3500 & size_lag<$s  & hclust>=$clust, robust cluster(cluster)
	coefplot, vertical keep(H_*)
...
*	xi: reg size_ch *_small *_large small h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
...
*	xi: reg ad_ch *_small *_large small h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
...
*	xi: reg ct_ch *_small *_large small h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
*	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch_small h_ch_large small) label append nocons  addtext(Treated Area, Over 5)	

	xi: reg size_ch h_ch h_ch_large  m_ch m_ch_large large    i.r*i.prov  if  hclust>=0 & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	...
	xi: reg size_ch h_ch h_ch_large  m_ch m_ch_large large h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	...
	xi: reg size_ch h_ch h_ch_large  m_ch m_ch_large large h_chS h_chML    i.r*i.prov  if  hclust>=10 & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	...
	xi: reg crowd_ch h_ch h_ch_large  m_ch m_ch_large large h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg ad_ch h_ch h_ch_large  m_ch m_ch_large large h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg ct_ch h_ch h_ch_large m_ch m_ch_large large h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
	outreg2 using clean/tables/size, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	

	xi: reg o_ch *_small *_large small size_lag h_chS h_chML    i.r*i.prov  if  hclust>=$clust & im<=$im & a<100 & size_lag<12, robust cluster(cluster)	
end
...
program define reduced_form
	* REDUCED FORM *
	...
	graph export clean/tables/weight_ch.pdf, as(pdf) replace
			...
			* BMI MEASUREMENTS
*	xi: reg zbmi_ch $controls a sex  h_chS h_chML    i.r*i.prov zbmi_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s &  zbmi_ch<3.4 & zbmi_ch>-2.4 & size_lag>2, robust cluster(cluster)	
...
*	drop if h_ch==1 & own==0
	...
	xi: reg zwfa_ch h_ch h_ch_large m_ch m_ch_large large  a sex  h_chS h_chML *_LARGE    i.r*i.prov  zwfa_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s & zwfa_ch<2.5 & zwfa_ch>-2.66 & size_lag>2, robust cluster(cluster)	
...
*	xi: reg zbmi_ch h_ch h_ch_large  m_ch m_ch_large large  a sex h_chS h_chML    i.r*i.prov zbmi_p*  if  hclust>=$clust & im<=$im & a<$a & size_lag<$s &  zbmi_ch<3.4 & zbmi_ch>-2.4  & size_lag>1, robust cluster(cluster)	
*	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	
	xi: reg c_health_ch h_ch h_ch_large m_ch m_ch_large large   h_chS h_chML *_LARGE    i.r*i.prov  a sex c_health_p* if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(cluster)	
	quietly sum c_health, detail
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)	 addstat(Mean, r(mean)) 
	xi: reg c_ill_ch h_ch h_ch_large m_ch m_ch_large large a sex c_ill_p*   h_chS h_chML *_LARGE    i.r*i.prov  if  hclust>=$clust & im<=$im & a<10 & size_lag<$s & size_lag>2, robust cluster(cluster)	
	quietly sum c_ill, detail
	outreg2 using clean/tables/health, nonotes tex(frag) keep(h_ch h_ch_large large) label append nocons  addtext(Treated Area, Over 5)  addstat(Mean, r(mean)) 
	
		** APPENDIX ** 
	xi: reg zhfa_ch $controls a sex h_chS h_chML    i.r*i.prov zhfa_p*  if  (zhfa_p25==0 & zhfa_p50==0) & hclust>=$clust & im<=3500 & a<10 & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.2  & size_lag>2, robust cluster(cluster)	
	coefplot, vertical keep ($graphs)
	xi: reg zhfa_ch $controls  a sex h_chS h_chML    i.r*i.prov zhfa_p*  if  (zhfa_p25==1 | zhfa_p50==1) &  hclust>=$clust & im<=3500 & a<10 & size_lag<$s & zhfa_ch<2 & zhfa_ch>-1.2  & size_lag>2, robust cluster(cluster)	
	coefplot, vertical keep ($graphs)
				* gains are consistent across previously malnourished kids (unfortunately )
	xi: reg zwfa_ch  $controls h_chS h_chML    i.r*i.prov zwfa_p*  if (zwfa_p25==0 & zwfa_p50==0) & hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zwfa_ch<1.4 & zwfa_ch>-1.6 & size_lag>2, robust cluster(cluster)	
	coefplot, vertical keep ($graphs)
	xi: reg zwfa_ch  $controls h_chS h_chML    i.r*i.prov zwfa_p*  if (zwfa_p25==1 | zwfa_p50==1) & hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zwfa_ch<1.4 & zwfa_ch>-1.6 & size_lag>2, robust cluster(cluster)	
	coefplot, vertical keep ($graphs)
				* pretty consistent across measures! * not the sickest kids gaining..
*	xi: reg zbmi_ch $controls  h_chS h_chML    i.r*i.prov zbmi_p*  if (zbmi_p25==0 & zbmi_p50==0) & hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zbmi_ch<1.6 & zbmi_ch>-1.44 & size_lag>2, robust cluster(cluster)	
*	coefplot, vertical keep ($graphs)
*	xi: reg zbmi_ch $controls  h_chS h_chML    i.r*i.prov zbmi_p*  if (zbmi_p25==1 | zbmi_p50==1) & hclust>=$clust & im<=3500 & a<$a & size_lag<$s & zbmi_ch<1.6 & zbmi_ch>-1.44 & size_lag>2, robust cluster(cluster)	
*	coefplot, vertical keep ($graphs)
...
program define income 
	...
	xi: reg pi_hhincome_ch h_ch  m_ch   h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	...
	xi: reg pi_hhincome_p_ch h_ch   m_ch   h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	...
	xi: reg pi_hhwage_ch h_ch  m_ch h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum pi_hhwage, detail
	outreg2 using clean/tables/inc, nonotes tex(frag) keep( h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg pi_hhwage_p_ch h_ch  m_ch   h_chS h_chML    i.r*i.prov  if   hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum pi_hhwage_p, detail
	outreg2 using clean/tables/inc, nonotes tex(frag) keep( h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg e_ch h_ch  m_ch  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum e, detail
	outreg2 using clean/tables/inc, nonotes tex(frag) keep( h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg ue_ch h_ch   m_ch   h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum ue, detail
	outreg2 using clean/tables/inc, nonotes tex(frag) keep( h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)

	*** WITH INTERACTION
	xi: reg pi_hhincome_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum pi_hhincome, detail
	...
	xi: reg pi_hhincome_p_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum pi_hhincome_p, detail
	...
	xi: reg pi_hhwage_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum pi_hhwage, detail
	outreg2 using clean/tables/inc_i, nonotes tex(frag) keep( h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg pi_hhwage_p_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum pi_hhwage_p, detail
	outreg2 using clean/tables/inc_i, nonotes tex(frag) keep( h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg e_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum e, detail
	outreg2 using clean/tables/inc_i, nonotes tex(frag) keep( h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg ue_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum ue, detail
	outreg2 using clean/tables/inc_i, nonotes tex(frag) keep( h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
...
program define remit
	...
	xi: reg a_cr_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	...
	xi: reg a_cg_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	...
	xi: reg pi_hhremitt_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if pi_hhremitt_ch<12000 & pi_hhremitt_ch>-12000 &  hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	outreg2 using clean/tables/remit, nonotes tex(frag) keep( h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over $clust)
	xi: reg pi_hhremitt_p_ch  h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if pi_hhremitt_p_ch<7000 & pi_hhremitt_p_ch>-7000 & hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	outreg2 using clean/tables/remit, nonotes tex(frag) keep( h_ch h_ch_large large  ) label append nocons  addtext(Treated Area, Over $clust)
...
** WITHOUT INTERACTION 
	xi: reg exp1_ch1 h_ch   m_ch  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	...
	xi: reg exp1_p_ch h_ch   m_ch  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	...
	xi: reg food_ch1 h_ch  m_ch   h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum food, detail
	outreg2 using clean/tables/exp, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons addstat(Mean, r(mean)) addtext(Treated Area, Over $clust)
	xi: reg food_p_ch h_ch  m_ch  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum food_p, detail
	outreg2 using clean/tables/exp, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons addstat(Mean, r(mean)) addtext(Treated Area, Over $clust)

	xi: reg non_food_1_ch1 h_ch  m_ch   h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum non_food_1, detail
	outreg2 using clean/tables/exp, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons addstat(Mean, r(mean)) addtext(Treated Area, Over $clust)
	xi: reg non_food_1_p_ch  h_ch   m_ch   h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum non_food_1_p, detail
	outreg2 using clean/tables/exp, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons addstat(Mean, r(mean)) addtext(Treated Area, Over $clust)

** WITH INTERACTION
	xi: reg exp1_ch1 h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum exp1, detail
	...
	xi: reg exp1_p_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum exp1_p, detail
	...
	xi: reg food_ch1 h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum food, detail
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg food_p_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum food_p, detail
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)

	xi: reg non_food_1_ch1 h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum non_food_1, detail
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large  ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)
	xi: reg non_food_1_p_ch  h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
	quietly sum non_food_1_p, detail
	outreg2 using clean/tables/exp_i, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addstat(Mean, r(mean))  addtext(Treated Area, Over $clust)

*	xi: reg kid_exp_ch1  h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s & kid_exp_ch<1000 & kid_exp_ch >-1000, robust cluster(cluster)	
...
*	xi: reg kid_exp_p_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s & kid_exp_ch<700 & kid_exp_ch >-700, robust cluster(cluster)	
*	outreg2 using clean/tables/exp1, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over $clust)

*	xi: reg rent_pay_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s, robust cluster(cluster)	
*	outreg2 using clean/tables/exp1, nonotes tex(frag) keep(h_ch h_ch_large large ) label append nocons  addtext(Treated Area, Over $clust)

*	xi: reg exp_n_ch1 h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s & exp_n_ch<2500 & exp_n_ch>-2500, robust cluster(cluster)	
...
*	xi: reg exp_n_p_ch h_ch h_ch_large  m_ch m_ch_large large  h_chS h_chML    i.r*i.prov  if hclust>=$clust &  im<=$im & a<$a & size_lag<$s &  exp_n_p_ch<1000 & exp_n_p_ch>-1000, robust cluster(cluster)	
