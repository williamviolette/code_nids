...
*	hist a, by(noch)
	...
	xtreg `var' noch a sex if a<=18 & r<3, fe robust
	}
	...
	********************************
	...
	g ratio1=hh1d/size_a
	g ratio2=hh1d/size
	
	tab ratio1 r if size_ch>0 & h_ch==1
	tab ratio2 r if size_ch>0 & h_ch==1
	
	egen hmax=max(ratio1), by(hhid)
	egen hmin=min(ratio1), by(hhid)
	g split=hmax!=hmin
	
	g min=ratio1==hmin
	g max=ratio1==hmax
	
*	hist a if split==1 & h_ch==1, by (min)
	
	tab a min if split==1 & h_ch==1
	
	tab rid min if split==1 & h_ch==1
	
	tab rid min if split==1 & h_ch==1 & a<=3 & r==3
		* why are there so many babies?!
	tab split if size_ch>0 & h_ch==0
	
	
	tab ratio1 r if size_ch>0 & h_ch==0	
	tab ratio1 r if size_ch<0
	
	
	egen hch=max(h_ch), by(hhid r)
	

	
	tab rid h_ch if r==2, mis
	
	tab rid h_ch if r==3, mis
	
	g i=1
	egen size_a=sum(i), by(hhid)
	
	
	xi: reg child_ch i.h_ch*size_lag i.r, cluster(hh1) robust
	
	xi: reg child_ch i.h_ch*size_lag i.r if size_lag<=6, cluster(hh1) robust
	
	**
	xi: reg size_ch i.h_ch*size_lag i.r if size_lag<=6, cluster(hh1) robust
	xi: reg size_ch i.h_ch*size_lag i.r, cluster(hh1) robust
	
	xi: reg adult_ch i.h_ch*size_lag i.r if size_lag<=6, cluster(hh1) robust
	
			* are they just not declining as much?
