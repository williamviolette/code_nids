...
** MORE FIXING NEEDS TO BE DONE WITH CONSTRUCT_HH1 !!!!!!
...
* 	quietly house_data
	use clean/data_v1.dta, clear
		replace hhgeo2011=geo2011 if r==4 | r==5
...
*  	keep if (best_race==1 | best_race==2 | best_race==4)
	...
	quietly change_variables
 	...
 	quietly oid
	quietly drop_rdp_leavers_fix
	cap drop mdb
	g mdb=hhmdbdc2011 if r==1
	replace mdb=gc_mdbdc2011 if r==2 | r==3
	replace mdb=mdbdc2011 if r==4 | r==5
...
program define school
	...
	foreach var of varlist edlstm_ltrr07 edlstm_ltrr08 edlstm_ltrr09 edlstm_ltrr10 edlstm_quin edlstm_nofee c_ed08curgrd c_ed10curgrd c_ed12curgrd  a_ed08att a_ed10att a_ed11att {
	replace `var'=. if `var'<0
	}
	foreach var of varlist a_ed08att a_ed10att a_ed11att {
	replace `var'=0 if `var'==2
	}
...
*	replace size=. if size>13
	...
	quietly g `var'_lnp=ln(`var'+1)/size
	quietly g `var'_lna=ln(`var'+1)/adult
	quietly g `var'_p=`var'/size
	quietly g `var'_a=`var'/adult
	...
	quietly by pid: g `var'_ln_a_ch=`var'_lna[_n]-`var'_lna[_n-1]
	quietly by pid: g `var'_e_ch=`var'_e[_n]-`var'_e[_n-1]
	quietly by pid: g `var'_p_ch=`var'_p[_n]-`var'_p[_n-1]
	quietly by pid: g `var'_a_ch=`var'_a[_n]-`var'_a[_n-1]
	}
...
program define child_count_res
	forvalues r=1/16 {	
	g c`r'_resss=(a_bhali`r'==1 & a_bhlive`r'==1 & a_bhdob_y`r'>=1990 & a_bhdob_y`r'<2013)
	g c`r'_nresss=(a_bhali`r'==1 & a_bhlive`r'!=1 & a_bhdob_y`r'>=1990 & a_bhdob_y`r'<2013)
	g c`r'_ressy=(a_bhali`r'==1 & a_bhlive`r'==1 & a_bhdob_y`r'>=1998 & a_bhdob_y`r'<2013)
	g c`r'_nressy=(a_bhali`r'==1 & a_bhlive`r'!=1 & a_bhdob_y`r'>=1998 & a_bhdob_y`r'<2013)
	g c`r'_ressvy=(a_bhali`r'==1 & a_bhlive`r'==1 & a_bhdob_y`r'>=2001 & a_bhdob_y`r'<2013)
	g c`r'_nressvy=(a_bhali`r'==1 & a_bhlive`r'!=1 & a_bhdob_y`r'>=2001 & a_bhdob_y`r'<2013)
	}
...
program define drop_rdp_leavers_fix
	g h_ch_orig=h_ch
	g h_chr=h_ch
	...
	replace h_ch=0 if rdp_r1m==1
	replace h_ch=0 if h_ch<0
	g h_chL=h_ch<0
	egen max_h_ch_hhid=max(h_ch), by(hhid)
	egen max_h_ch_hhid_pid=max(max_h_ch_hhid), by(pid)
	replace h_ch=0 if max_rdp==1 & max_h_ch<1 & max_h_ch_hhid_pid<1
	g h_chML=(max_rdp==1 & max_h_ch<1 & max_h_ch_hhid_pid<1)
	replace h_ch=0 if h_ch==. & r>1
end
...
program define construct_hh1
	...
	* alternate hhid for pid
	...
	egen h4=max(hhid4), by(pid)
	egen h5=max(hhid5), by(pid)
	* define hh1
	g hh1=h1
	replace hh1=h2alt if hh1==. & hchid2!=1 & r>=2
	replace hh1=h2 if hh1==.
	replace hh1=h3 if hh1==.
	replace hh1=h4 if hh1==.
	replace hh1=h5 if hh1==.
	* fix hh1
end
