...
outreg2 using paper/first_stage_v1, tex(frag) label replace keep(rdp_np rdp_op) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES) title("Head of Household")

foreach c in g_hoh m_f_res {
quietly xi: xtreg `c' rdp_op rdp_np i.r if max_age<16, cluster(hh1) fe robust
outreg2 using paper/first_stage_v1, tex(frag) label append keep(rdp_np rdp_op) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES)
}

*** DEMOGRAPHICS ***
quietly xi: xtreg size rdp_op rdp_np i.r if max_age<16, cluster(hh1) fe robust
outreg2 using paper/first_stage_v2, tex(frag) label replace keep(rdp_np rdp_op) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES) title("Demographics")

foreach c in  child_alt inc {
quietly xi: xtreg `c' rdp_op rdp_np i.r if max_age<16, cluster(hh1) fe robust
outreg2 using paper/first_stage_v2, tex(frag) label append keep(rdp_np rdp_op) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES)
}
...
*** reduced form v1
quietly xi: xtreg absent rdp_op rdp_np i.r if max_age<16, fe  robust
outreg2 using paper/reduced_form_v1, tex(frag) label replace keep(rdp_np rdp_op) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES) title("Education Impacts: Attendance")

quietly xi: xtreg absent_i rdp_op rdp_np i.r if max_age<16, cluster(hh1) fe robust
outreg2 using paper/reduced_form_v1, tex(frag) label append keep(rdp_np rdp_op) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES)
...
quietly xi: xtreg c_failed rdp_op rdp_np i.r if max_age<16 & max_age>10, cluster(hh1) fe robust
outreg2 using paper/reduced_form_v2, tex(frag) label replace keep(rdp_np rdp_op) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES, Maximum Age 11 to 15, YES) title("Education Impacts: Continued")

quietly xi: xtreg p_pay rdp_op rdp_np i.r if max_age<16, cluster(hh1) fe robust
outreg2 using paper/reduced_form_v2, tex(frag) label append keep(rdp_np rdp_op) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES)
...
outreg2 using paper/mech_v1, tex(frag) label replace keep(rdp_np rdp_op) nocons addtext( Young Child Present, YES, Time Fixed Effects, YES, Individual Fixed Effects, YES) title("Child Care Mechanism") addnote("Note: Absent is measured in days absent greater than one (Intensive Margin)")
quietly xi: xtreg absent_i rdp_op rdp_np i.r if max_age<16 & youngr1==0, cluster(hh1) fe robust
outreg2 using paper/mech_v1, tex(frag) label append keep(rdp_np rdp_op) nocons addtext( Young Child Present, NO, Time Fixed Effects, YES, Individual Fixed Effects, YES) title("Child Care Mechanism")
...
outreg2 using paper/house_quality_v1, tex(frag) label replace keep(rdp_op) nocons addtext(Time Fixed Effects, YES, Urban/Rural Control, YES) title("RDP House Quality across Parent and Non-Parent Owners") addnote("Limit Sample to RDP Houses (non-parent owners are the reference group)")
foreach v in piped elec mktv {
quietly xi: reg `v' rdp_op u i.r if max_age<16 & max_age>3 & (rdp_op==1 | rdp_np==1), cluster(hh1) robust
outreg2 using paper/house_quality_v1, tex(frag) label append keep(rdp_op) nocons addtext(Time Fixed Effects, YES, Urban/Rural Control, YES)
}
...
outreg2 using paper/school_quality_v1, tex(frag) label replace keep(rdp_np rdp_op) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES) title("School Quality")

foreach c in class_size sch_q sch_d {
quietly xi: xtreg `c' rdp_op rdp_np i.r if max_age<16, cluster(hh1) fe robust
outreg2 using paper/school_quality_v1, tex(frag) label append keep(rdp_np rdp_op) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES)
}
...
outreg2 using paper/robust_v1, tex(frag) label replace keep(rdp_np rdp_op m_res f_res size child_alt) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES) title("Alternative Controls") addnote("Note: Absent is measured in days absent greater than one (Intensive Margin)")

quietly xi: xtreg absent_i rdp_op rdp_np m_res f_res size child_alt si_* i.r if max_age<16, cluster(hh1) fe robust
outreg2 using paper/robust_v1, tex(frag) label append keep(rdp_np rdp_op m_res f_res size child_alt) nocons addtext(All Demographics, YES, Time Fixed Effects, YES, Individual Fixed Effects, YES)

quietly xi: xtreg absent_i rdp_op rdp_np sch_d i.r if max_age<16, cluster(hh1) fe robust
outreg2 using paper/robust_v1, tex(frag) label append keep(rdp_np rdp_op m_res f_res size child_alt sch_d) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES) 
...
outreg2 using paper/robust_v1, tex(frag) label append keep(rdp_np rdp_op m_res f_res size child_alt sch_q) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES)

quietly xi: xtreg absent_i rdp_op rdp_np i.r if max_age<16 & sq==0, cluster(hh1) fe robust
outreg2 using paper/robust_v1, tex(frag) label append keep(rdp_np rdp_op m_res f_res size child_alt sch_d) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES, No Change in Sch Quin, YES)

quietly xi: xtreg absent_i rdp_op rdp_np i.r if max_age<16 & sq==1, cluster(hh1) fe robust
outreg2 using paper/robust_v1, tex(frag) label append keep(rdp_np rdp_op m_res f_res size child_alt sch_d) nocons addtext(Time Fixed Effects, YES, Individual Fixed Effects, YES, Some Change in Sch Quin, YES)
