...
xi: xtreg min_age rdp_np rdp_op i.r if  max_age<16 & max_age>8, cluster(hh1) fe robust
* able to take young ones in? minimum age declines in op...

xi: xtreg p_hoh a sex c_att1 absent c_edu1 u i.r if max_age<16 & (p_hoh==1 | g_hoh==1), cluster(hh1) robust
** older kids live with parents; shock moves younger kids to live with parents
** ** uncorrelated with education variables oddly.. 
** ** ** which kids live with parents?

** no difference in age
reg a rdp_op u i.r if (rdp_op==1 | rdp_np==1) & max_age<16, cluster(hh1) robust
* older kids go with parents
reg rdp_op a sex c_att1 absent c_edu1 u i.r if (rdp_op==1 | rdp_np==1) & max_age<16, cluster(hh1) robust
* kids that attend school are more likely to live with their parents in RDP's: suggestive evidence for treatment effect


reg c_att1 rdp_op a u i.r if (rdp_op==1 | rdp_np==1), cluster(hh1) robust


*** SYSTEMATIC DIFFERENCES IN HOUSE ATTRIBUTES? ***
******** NOPE ********
...
** UNCORRELATED WITH YOUNG
xi: xtreg young rdp_np rdp_op i.r if  max_age<16, cluster(hh1) fe robust
...
** minimum age in household, is correlated with absent.. BUT results are still robust
xi: xtreg absent rdp_np rdp_op m_res f_res size child_alt min_age i.r if max_age<16 & max_age>6 & absent>0, cluster(hh1) fe robust

** PRESENCE OF YOUNG CHILD
xi: xtreg absent rdp_np rdp_op i.r if max_age<16 & max_age>8 & absent>0 & youngr1==1, cluster(hh1) fe robust
xi: xtreg absent rdp_np rdp_op i.r if max_age<16 & max_age>8 & absent>0 & youngr1==0, cluster(hh1) fe robust

xi: xtreg absent_d rdp_np rdp_op i.r if max_age<16 & max_age>8 & youngr1==1, cluster(hh1) fe robust
xi: xtreg absent_d rdp_np rdp_op i.r if max_age<16 & max_age>8 & youngr1==0, cluster(hh1) fe robust
...
xi: xtreg p_pay rdp_np rdp_op i.r if  max_age<16 & max_age>10, cluster(hh1) fe robust
